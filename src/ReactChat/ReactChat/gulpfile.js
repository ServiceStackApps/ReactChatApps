...
(function () {
    ...
    var winFormsAssemblyInfoPath = '../ReactChat.AppWinForms/Properties/AssemblyInfo.cs';
    ...
    gulp.task('www-msbuild-web', function() {
        return gulp.src('./ReactChat.csproj')
            .pipe(msbuild({
    ...
    gulp.task('www-msbuild-resources', function () {
        return gulp.src('../ReactChat.Resources/ReactChat.Resources.csproj')
            .pipe(msbuild({
                targets: ['Clean', 'Rebuild'],
                properties: {
                    Configuration: 'Release'
    ...
    gulp.task('www-msbuild-console', function () {
        return gulp.src('../ReactChat.AppConsole/ReactChat.AppConsole.csproj')
            .pipe(msbuild({
                targets: ['Clean', 'Rebuild'],
                properties: {
                    Configuration: 'Release'
...
gulp.task('www-nuget-pack-winforms', function (callback) {
        ...
        var excludes = globule.find([
            '../ReactChat.AppWinForms/bin/x86/Release/*.pdb',
            '../ReactChat.AppWinForms/bin/x86/Release/*.vshost.*'
        ]);
        var includes = [
        { src: '../ReactChat.AppWinForms/bin/x86/Release/*.*', dest: '/lib/net45/' },
        { src: '../ReactChat.AppWinForms/bin/x86/Release/locales/*.*', dest: '/lib/net45/locales/' }];
        return nugetpack({
            id: title,
            title: title,
                version: version,
